...
// 1. caleenger에 스테이지에 도전하는 사용자수를 저장. 배열의 킉를 N + 2로 정한 이유 -> N번째 스테이지를 클리어한 사용자는 stage가 N+1dlamfh challenger 배열에서 N+1 위치에 데이터를 저장해야 ㅎ나ㅡㄴ데 배열의 인덱스는 0부터 시작이므로 N+1 인덱스에 데이터를 저장하려면 N+2 크기의 배열ㅇ ㅣ필요하기 때문입니다.